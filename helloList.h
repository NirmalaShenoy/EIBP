...
extern void modify_LL(char *addr,char recvOnEtherPort[5]);
extern void modify_LL_updated(char labels[][128], int totalEntries, char recvOnEtherPort[5], int myTier);
...
struct childInterfaceToLabelList {
    char interface[10]; // Interface on which the join request was received form the child tier
    struct labels* labelList; // Linked list of labels assigned to this interface
    struct childInterfaceToLabelList* next; // Pointer to the next node in the list
}*headInterfaceLabel;
...
extern char failedInterface[20];
/***********************************************************
 * append()
 ...
 *******************************************************************************/
void printNeighbourTable(bool isRemoved) {
	if(isRemoved)
	{
	...
	if(enableLogScreen)
	   // gettimeofday(&down_time, NULL);
		...
		printf("\n\n*************** Neighbour Table ************* CURRENT_TIME:%f\n",getCurrentTime());
 ...
 int staging(char inPort[20]){
	...
	if(checkstableport == 1){
		...
		if(checkStaging==1){
			...
			temp2->lastUpdate = getCurrentTime();
			temp2->countHello=1;
		...
		//printf("Here to update time2\n");
				if (strlen(uNode->port) == strlen(inPort)) {
					...
					if (strncmp(uNode->port, inPort, strlen(inPort)) == 0) {
						...
						uNode->lastUpdate = getCurrentTime();
	...
	while (uNode != NULL) { 
				if (strlen(uNode->port) == strlen(inPort)) {
					if (strncmp(uNode->port, inPort, strlen(inPort)) == 0) {
						...
						//printf("TEST: Message Counter %d\n", uNode->countHello);
						printf("\nHello message received on interface: %s CURRENT_TIME:%lf\n", inPort,getCurrentTime()); 
						//break;
						if(uNode->countHello>=3){
						 	 printf("\nMessageCount >= 3 : %s CURRENT_TIME:%lf\n",inPort,getCurrentTime());
							 printf("\nHello message received on interface: %s CURRENT_TIME:%lf\n", inPort,getCurrentTime()); 
							stable=1;
...
*****************************************************************/
int deleteMyLabelsRelated(char tier[20],char recvOnEtherPort[5]){
	printf("\nIn delete My Labels related \n");
	struct nodeTL *temp = headTL;
	int ret = 0;
	while(temp){
		if(checkIfSubstr(temp->tier,tier)){
			//modify_LL(temp->tier,recvOnEtherPort);
 ...
 **************************************************************************/
int delete(char recvOnEtherPort[5]) {
	struct stablePort *temp = headStable;
	struct stablePort *prev = headStable;
	//struct nodeTL *fNode = ; //  - head of the My label table
	...
	while (temp != NULL) {
		...
		if (delTimeDiff >= 1.40) { //dead timer(from 1.3 to 0.40) reduced 4/20/2022 - Samruddhi
			...
			//printf("\nmatchPort %d",match_Port);
			if(t==0){
				//printf("\nInterface down at %s",temp->port);
				//printf("\nmatch port: %d",match_Port);
				//gettimeofday(&down_time, NULL);
				//d_time = ((double)down_time.tv_sec*1000000 + (double)down_time.tv_usec) / 1000000;
				printf("\nIF_TIMER_DOWN:%lf\n", getCurrentTime());
				printf("\nfailedInterface:%s\n", temp->port);
				strcpy(failedInterface,temp->port); 
				t = 1;
				//printf("\nmatch_port:%d\n", match_Port);
				if(match_Port==2){
				...
				if (strlen(temp2->port) == strlen(temp->port)) {
					if (strncmp(temp2->port, temp->port, strlen(temp->port)) == 0) {
						...
						if(match_Port==3){
							...
							deleteMyLabelsRelated(temp2->tier,recvOnEtherPort);
						}
						if(match_Port==2){
							
							//deleteIPLabel(temp->port,recvOnEtherPort);
						}
						ret= 1;
					}else{
						prev2=temp2;
 ...
 *******************************************************************************/
int findFailed_LL(char recvOnEtherPort[5]) {
...
double getCurrentTime()
{
	...
	double c_time;
	gettimeofday(&current_time, NULL);
	c_time = ((double)current_time.tv_sec*1000000 + (double)current_time.tv_usec) / 1000000;
	return c_time;
}
