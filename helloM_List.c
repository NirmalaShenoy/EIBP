...
bool deleteIPLabel(char *newLabel, char recvOnEtherPort[5]) {
    ...
    if (myTierValue == 1) {
        ...
        if (failedLL_head != NULL) {
            setInterfaces();  // Interface list is initialized here
            uint8_t* mplrPayload = allocate_ustrmem(IP_MAXPACKET);

            if (!mplrPayload) {
                printf("Memory allocation failed for MPLR payload.\n");
                return foundChild;
            }
			printf("before buildPayloadRemoveAdvts5\n");
...
void msg_typ_labl_lst(char buffer[2048], unsigned char* ethhead, char recvOnEtherPort[5], struct sockaddr_ll src_addr) {
   ...
   // d_time = ((double)down_time.tv_sec * 1000000 + (double)down_time.tv_usec) / 1000000;
    ...
    uint8_t totalEntries = ethhead[15];
    int index = 16;
	int entriesToSend = totalEntries;
    ...
    for (int i = 0; i < totalEntries; i++) {
        uint8_t labelLength = ethhead[index++];
        memcpy(labels[i], &ethhead[index], labelLength);
        labels[i][labelLength] = '\0'; // Null-terminate the label string
        index += labelLength;
        printf("Label %d: %s\n", i + 1, labels[i]);
		...
		if (!is_label_processed(labels[i])) {
			...
			entriesToSend=entriesToSend-1;
			//printf("entriesToSend %d",entriesToSend);
			//free(labels[i]);
		}

    }
	if(newLabelCount==0){
		printf("\nAll labels are already processed.\n");
	}

	modify_LL_updated(labels, totalEntries, recvOnEtherPort,myTierValue);
    // Handle the rest of the logic for processing failedLL_head
    if (failedLL_head != NULL) {
        setInterfaces(); // interfaceList is being set by this function
        int loopCounter2 = 1; // Don't send on eth0, hence start from 1
        uint8_t* mplrPayload = allocate_ustrmem(IP_MAXPACKET);
        int mplrPayloadLen = 0;
        int port_number = 0;
		printf("before sending buildPayloadRemoveAdvts1");
        mplrPayloadLen = buildPayloadRemoveAdvts(mplrPayload, failedLL_head, 4);
		//printf("after sending MESSAGE_TYPE_ENDNW");
        if (mplrPayloadLen) {
				...
				for (int i = 0; i < newLabelCount; i++) {
        			...
        			if (tier == 2 && port) {
            			if (isDerivedLabel(middleSegment)) {
							...
							char **allCollectedLabels = NULL;
        					int totalCollectedCount = 0;
							isRemoved = delete_neighbour_by_failedPort(port);
							deleteLabelsAndCheckGrandparents(newLabels, newLabelCount, &totalDeletedCount);
							/*
                			if (collect_neighbours_by_failedPort(port, &allCollectedLabels, &totalCollectedCount)) {
            					printf("Collected %d labels\n", totalCollectedCount);

            					// Now call the deleteLabelsAndCheckGrandparents function with the collected labels
            					char **allDeletedLabels = deleteLabelsAndCheckGrandparents(allCollectedLabels, totalCollectedCount, &totalDeletedCount);
    ...
    for (int i = 0; i < labelCount; i++) {
        ...
        while (currentNeighbor != NULL) {
            if (strcmp(currentNeighbor->tier, label) == 0) {
                printf("\nRemoving label from Neighbor Table: %s\n", currentNeighbor->tier);
				isRemoved = true;
                // Store label
